...
namespace Assets.Scripts.Game.UI.Controller
{
	public class LeftPanelWindow: WindowController
	{
		...
		{
			if (isCheck)
				_context.Publisher.Publish<SetInputStateEvent, eInputState>(eInputState.Create);
		}

		private void OnSetMoveState(bool isCheck)
		{
			if (isCheck)
				_context.Publisher.Publish<SetInputStateEvent, eInputState>(eInputState.Move);
		}

		private void OnAttackState(bool isCheck)
		{
			if (isCheck)
				_context.Publisher.Publish<SetInputStateEvent, eInputState>(eInputState.Attack);
		}
	}
}
